function model = single_experiment(tfpr, data_name, test_repeat, optimized_params)
    ...
    hyperparams.tree_depth = [4];
    hyperparams.split_prob = 0.5;
    hyperparams.node_loss_constant = [4];
    hyperparams.entropy_th = [0.6];
    hyperparams.explore_pr = [0.3];
    
    % generate hyper-parameter space 
    hyperparam_space = utility_functions.generate_hyperparameter_space_active_Tree_OLNP(hyperparams);
    ...
    if isempty(optimized_params)
        ...
        if hyperparam_number>1
            ...
            for i=1:length(hyperparam_space)
                ...
                parfor j=1:cross_val_MC
                    ...
                    entropy_th = hyperparam_space{i}.entropy_th;
                    explore_pr = hyperparam_space{i}.explore_pr;
                    % load the model
                    model = active_Tree_OLNP(eta_init, beta_init, gamma, sigmoid_h, lambda, tree_depth, split_prob, node_loss_constant, entropy_th, explore_pr, n_features, tfpr);
            ...
            entropy_th = hyperparam_space{target_hyperparameter_index}.entropy_th;
            explore_pr = hyperparam_space{target_hyperparameter_index}.explore_pr;
            ...
            entropy_th = hyperparam_space{1}.entropy_th;
            explore_pr = hyperparam_space{1}.explore_pr;
        ...
        entropy_th = optimized_params.entropy_th;
        explore_pr = optimized_params.explore_pr;
