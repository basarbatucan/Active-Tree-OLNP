classdef active_Tree_OLNP
    ...
    methods
        ...
        function obj = active_Tree_OLNP(eta_init, beta_init, gamma, sigmoid_h, lambda, tree_depth, split_prob, node_loss_constant, entropy_th, explore_pr, n_features, tfpr)
            ...
            obj.entropy_th_ = entropy_th;
            obj.explore_pr_ = explore_pr;
        ...
        function obj = train(obj, X_train, y_train, X_test, y_test, test_repeat)
            ...
            entropy_th = obj.entropy_th_;
            explore_pr = obj.explore_pr_;
            ...
            tp = 0;
            fp = 0;
            test_period = 1000; % do test every 1000 sample training 
            ...
            % active learning related updates
            active_learning_sample_index = 1;
            test_counter = test_period-1;
            test_index = 1;
            ...
            for i=1:n_samples_train
                ...
                % based on the predictions make decision about active
                % learning
                
                p_pos = 0;
                p_neg = 0;
                for k=1:length(dark_node_indices)
                    node_index = dark_node_indices(k);
                    node_prediction = C(node_index);
                    if node_prediction == 1
                        p_pos = p_pos + mu_tree(k);
                    else
                        p_neg = p_neg + mu_tree(k);
                    end
                end
                decision_statistics = -(p_pos*log2(p_pos)+p_neg*log2(p_neg));

                if isnan(decision_statistics)
                    decision_statistics = 0;
                end
                
                % First check for exploration probability
                % With this probability, we will not be considering the
                % explore part
                uni_random = rand;
                if (uni_random <= explore_pr)
                    % this sample will definitely be explored
                    use_the_sample_flag = 1;
                else
                    % sample is not part of the exploration
                    % so check for active learning criterion
                    if (decision_statistics > entropy_th)
                        % experts are not confident
                        % we should use this sample
                        use_the_sample_flag = 1;
                    else
                        % experts are confident, we can skip this sample
                        use_the_sample_flag = 0;
                    end
                end
                
                if use_the_sample_flag == 0
                    continue;
                end
                ...
                % save sample mu
                sample_mu(active_learning_sample_index, :) = mu_tree;
                ...
                tpr_train_array(active_learning_sample_index) = tp/number_of_positive_samples;
                fpr_train_array(active_learning_sample_index) = fp/number_of_negative_samples;

                % save gamma
                gamma_array(active_learning_sample_index) = gamma;
                ...
                if test_counter==0
                    ...
                    tpr_test_array(test_index) = tpr_tmp;

                    fp_tmp = sum(index_pred_pos & index_act_neg);
                    fpr_tmp = fp_tmp/N_act_neg;
                    fpr_test_array(test_index) = fpr_tmp;
                ...
                if yt==1
                    ...
                    % save class costs
                    pos_class_weight_train_array(active_learning_sample_index) = mu;
                    if active_learning_sample_index>1
                        neg_class_weight_train_array(active_learning_sample_index) = neg_class_weight_train_array(active_learning_sample_index-1);
                    end
                else
                    % mu(t) uses gamma(t-1), n_plus(t-1), n_minus(t-1)
                    mu = gamma*(number_of_positive_samples + number_of_negative_samples)/number_of_negative_samples;
                    % save class costs
                    neg_class_weight_train_array(active_learning_sample_index) = mu;
                    if active_learning_sample_index>1
                        pos_class_weight_train_array(active_learning_sample_index) = pos_class_weight_train_array(active_learning_sample_index-1);
                    end
                end
                ...
                % update active learning index counter
                active_learning_sample_index = active_learning_sample_index+1;
            ...
            % save the results
            obj.mu_train_array_ = sample_mu(1:active_learning_sample_index-1, :);
            obj.tpr_train_array_ = tpr_train_array(1:active_learning_sample_index-1);
            obj.fpr_train_array_ = fpr_train_array(1:active_learning_sample_index-1);
            obj.tpr_test_array_ = tpr_test_array(1:test_index-1);
            obj.fpr_test_array_ = fpr_test_array(1:test_index-1);
            obj.neg_class_weight_train_array_ = neg_class_weight_train_array(1:active_learning_sample_index-1);
            obj.pos_class_weight_train_array_ = pos_class_weight_train_array(1:active_learning_sample_index-1);
            obj.test_indices_ = (1:test_repeat)*test_period;
